        ...
        if vardim == 1:
            ...
            if (paths["LSM"] in filename) or (paths["TOPAZ"] in filename):
                ...
                if paths["AMSR2"] in filename:
                    if (var == "ice_conc") or (var == "total_standard_uncertainty"):
...
def SIC_trend(date_task, trend_period, dataset):
    Dataset = {}
...
def extract_SIC_obs_predictors(date_task, trend_period, TOPAZ, LSM, dataset, proj = proj, crs = crs):
    Data_TOPAZgrid = {}
    SIC_obs = SIC_trend(date_task, trend_period, dataset)
    #
    ...
    for var in variables[dataset]:
        if var != "total_standard_uncertainty":
