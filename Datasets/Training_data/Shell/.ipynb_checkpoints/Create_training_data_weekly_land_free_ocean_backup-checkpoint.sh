...
# extract_SIC_AMSR2_previous_day
# 
#        date_task: forecast start date (string "YYYYMMDD")
#        TOPAZ: TOPAZ dataset (dictionary)   
#        LSM: TOPAZ land sea mask (dictionary)
#        crs: crs defined in "Constants"
#        paths: paths defined in "Constants"

# In[15]:


def extract_SIC_AMSR2_previous_day(date_task, TOPAZ, LSM, proj = proj, crs = crs, paths = paths):
    Data_TOPAZgrid = {}
    previous_day = (datetime.datetime.strptime(date_task, "%Y%m%d") - datetime.timedelta(days = 1)).strftime("%Y%m%d")
    filename_AMSR2 = paths["AMSR2"] + previous_day[0:4] + "/" + previous_day[4:6] + "/sic_cosi-5km_" + previous_day + "0000-" + date_task + "0000.nc"
    SIC_AMSR2 = read_netCDF(filename_AMSR2, variables = ["ice_conc"])
    #
    transform_AMSR2_to_TOPAZ = pyproj.Transformer.from_crs(crs["AMSR2"], crs["TOPAZ"], always_xy = True)
    xx_AMSR2, yy_AMSR2 = np.meshgrid(SIC_AMSR2["x"], SIC_AMSR2["y"])
    xx_AMSR2_TOPAZproj, yy_AMSR2_TOPAZproj = transform_AMSR2_to_TOPAZ.transform(xx_AMSR2, yy_AMSR2)
    idx_fill_value = SIC_AMSR2["ice_conc"] < 0
    SIC_AMSR2["ice_conc"][idx_fill_value == True] = -32767
    Data_TOPAZgrid["SIC"] = nearest_neighbor_interp(xx_AMSR2_TOPAZproj, yy_AMSR2_TOPAZproj, TOPAZ["x"], TOPAZ["y"], np.squeeze(SIC_AMSR2["ice_conc"]), fill_value = -32767)
    Data_TOPAZgrid["SIC"][LSM["LSM"] == 0] = 0
    #
    return(Data_TOPAZgrid)
...
Datasets["TARGET"] = extract_targets(date_task = date_task, 
                                     ...
                                     paths = paths)
#
Datasets["SICobs_AMSR2"] = extract_SIC_AMSR2_previous_day(date_task = date_task, 
                                                          TOPAZ = Datasets["TOPAZ"], 
                                                          LSM = Datasets["LSM"], 
                                                          proj = proj, 
                                                          crs = crs, 
